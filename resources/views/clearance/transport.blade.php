...
@section('title', 'Transport Clearance Report')
@section('extrastyle')
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered">
                      ...
                      @foreach($students as $reg)
                        <tr>
                          ...
                          <td>{{$reg->Full_Name}}</td>   
                      <td>
                         <a onclick="return confirm('Make sure you are approving Transport clearance status!');"  id='{{$reg->id}}' href='{{URL::route('all.clearance',[$reg->student_id,'type'=>3])}}'>
                          <?php
                          if($reg->transport ==1){
