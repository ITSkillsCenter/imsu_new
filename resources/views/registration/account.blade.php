...
@section('title', 'Account clearance report')
@section('extrastyle')
            ...
            <div class="row">
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                        ...
                        <form action="{{route('department.clearance')}}" method="post">
                          ...
                          <div class="row">
                             ...
                             <select class="form-control   has-feedback-left" name="department" id="department_id" required>
                                 <option selected disable value=""> --Select Department--</option>
                             ...
                             <div class="col-md-2">
                                <div class="item form-group">
                                   ...
                                   <select class="form-control   has-feedback-left" name="semester" id="department_id" required>
                                    <option selected value="" disable> --Select Semester--</option>
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered">
                      ...
                      @foreach($registrations as $reg)
                        <tr>
                          ...
                          <td>{{$reg->Full_Name}}</td>   
                      <td>
                         <a onclick="return confirm('Make sure you are approving account clearance status!');"  id='{{$reg->id}}' href='{{URL::route('registration.clearance',[$reg->id,'type'=>3])}}'>
                          <?php
                  ...
                  <p>Showing results of semester <b class="label label-success">{{ Session::get('semester') }}</b> from the department of <b class="label label-success">{{ Session::get('department') }}</b> and registration type 
                  
                    <?php
                    if(Session::get('reg_type') ==1){
                      echo '<b class="label label-success"> Regular/ Term wise</b>';
                    }elseif(Session::get('reg_type') == 2){
                      echo '<b class="label label-primary"> Retake</b>';
                    }elseif(Session::get('reg_type') == 3){
                      echo '<b class="label label-info"> Improvement</b>';
                    }elseif(Session::get('reg_type') == 4){
                      echo '<b class="label label-info"> Backlog</b>';
                    }else{
                       echo '';
                    }
                  ?>
